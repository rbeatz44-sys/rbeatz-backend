...
audio.addEventListener('error', () => {
  ...
  if (err) {
    const codes = {1:'Playback aborted',2:'Network error',3:'File format not supported/corrupted',4:'File format not supported by this browser'};
    msg = codes[err.code] || `Error code ${err.code}`;
  }
  isPlaying = false; updatePlayerUI(); renderAll();
  alert('Could not load this track: ' + msg);
});
...
function playTrack(id){
  ...
  if(currentTrackId===id){
    if(isPlaying){ audio.pause(); isPlaying=false; updatePlayerUI(); renderAll(); }
    else {
      audio.play().then(()=>{ isPlaying=true; updatePlayerUI(); renderAll(); })
        .catch(err=>{ isPlaying=false; updatePlayerUI(); renderAll(); alert('Playback error: '+err.message); });
    }
  } else {
    currentTrackId=id; audio.src=t.objectPath; audio.loop=isLooping;
    audio.play().then(()=>{ isPlaying=true; updatePlayerUI(); renderAll(); })
      .catch(err=>{ isPlaying=false; updatePlayerUI(); renderAll(); alert('Playback error: '+err.message); });
  }
